...
def read_json(path):
    ...
    with codecs.open(f"{cwd}/{path}",encoding='utf-8') as f:
        data = json.load(f)
    return data
...
def generation(data):
    ...
    if CharacterName == "シャルロット":
        CharacterImage = CharacterImage.resize((int(CharacterImage.width*0.85),int(CharacterImage.height*0.85)))
    else:
    ...
    if CharacterName == 'アルハイゼン' or CharacterName == "ヌヴィレット":
    ...
    FriendShipLenght = D.textlength(str(FriendShip),font=config_font(25))
    D.rounded_rectangle((35+levellength+5,74,77+levellength+FriendShipLenght,102),radius=2,fill="black")
    ...
    for k,v in CharacterStatus.items():
        ...
        if k in ['HP','防御力','攻撃力']:
            HPpls,HPbase,HPsize,HPbsize = GenerateBaseText(k)
